/*
 * CSE 5306 Sprint 2014 - Project 2 - Ring Coordinator Election Algorithm
...
public class RingAlgorithm extends JFrame {
		...
		public static boolean isCoord = false, isCrashed = false, isIdle = true, crashFlag = false; // declare some useful boolean variables
		...
		public RingAlgorithm() {
			...
			btnElect.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					// This is to update the active process list into an array list whenever Manual Election button is clicked
					// ********** DE-SCOPED *********
					...
					for(int m=1;m<6;m++)
					{
						...
						tempPortNo = defPort + m;
						try{
							tempSkt= new ServerSocket(tempPortNo); // declare a new socket
	    ...
	    // ******** DE-SCOPED **********
	    /*private void nextAvailablePort(int procID) {
	    	if(procID>5) {
	        	procID = procID - 5;
	        }
	    	int availPort = defPort + procID;
	    	try {
				servSocket = new ServerSocket(availPort);
				procNo = procID;
				procNo = availPort;
				String tempProcess = Integer.toString(procNo);
				pNo.setText(tempProcess);
				textArea.append("\n  Crashed Port is busy, So Connected to the port : " + portNo);
				thread.informReset(portNo, procNo, servSocket);
				thread.resume();
			} catch (IOException e) {
				if (procID < 6) {
					nextAvailablePort(procID+1);
				} else {
...
class processThread extends Thread {
		                ...
		                case "COORDINATOR":  // action to be performed when it is an coordinator message
		                    ...
		                    try {
		                        ...
		                        if(Integer.parseInt(token3) == processNumber) {
		                        	...
		                        	if(!gui.isCoord) {  // Print the new coordinator info
		                        		gui.textArea.append("\n New Coordinator is --> " + token2 + "  Elected By " + token3);
		                        	}
		                        	gui.cNo.setText(token2); // update the GUI
    ...
    private void findCoordinator(int[] processes) {
      ...
      //Check the first process number and compare with other live processes and elect Coordinator with highest process number
        ...
        gui.cNo.setText(Integer.toString(newCoordinator));
